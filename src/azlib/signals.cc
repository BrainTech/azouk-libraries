...
#define AZOUK_SIGNALS_callback_name(SIGNAME) BOOST_PP_CAT(_, \
            BOOST_PP_CAT(SIGNAME, _posix_callback))
...
#define AZOUK_SIGNALS_export_and_create_c_callback(r, d, SIG) \
	...
	\
	/* statically bind AZOUK_SIGNALS_callback_name(SIG) to signal */ \
        /* AZOUK_SIGNALS_c_name(SIG) */ \
	...
	AZOUK_TRIGGER_STATIC_INITILIZATION_CODE_NAME(( \
		    ...
		    SIG->connect(std::numeric_limits<int>::min(), \
                        std::cerr << constant("signal " \
                            BOOST_PP_STRINGIZE(SIG) " called\n")); \
		    SIG->connect(std::numeric_limits<int>::min(), \
                        &BOOST_PP_CAT(print_that_called_, SIG)) \
	    ), BOOST_PP_CAT(print_that_called, SIG), true); \
	// and that's it

	BOOST_PP_SEQ_FOR_EACH(AZOUK_SIGNALS_export_and_create_c_callback, ~,
                AZOUK_SIGNALS_POSIX_SIGNALS_SEQ);
...
#define AZOUK_SIGNALS_quit_on(r, d, SIG) \
	...
	AZOUK_TRIGGER_STATIC_INITILIZATION_CODE_NAME( \
                (SIG->connect(std::numeric_limits<int>::max(), \
                              &BOOST_PP_CAT(exit_on_, SIG))), SIG, true);
	BOOST_PP_SEQ_FOR_EACH(AZOUK_SIGNALS_quit_on, ~,
                AZOUK_SIGNALS_QUIT_ON_SEQ);
#undef AZOUK_SIGNALS_quit_on
	...
	namespace impl {
		...
		if (count++ == 0) {
		    /*
                     * create SIGs before any other module initializers are
...
#undef AZOUK_SIGNALS_create_signal
		    ...
		    exit_signal->connect(std::numeric_limits<int>::max(), &exit,
                    ...
                    // when TERM or INT is called call exit_signal with proper
                    // status
#define AZOUK_SIGNALS_connect_killers(r, d, SIG) signals::SIG->connect( \
                        boost::bind(ref(*signals::exit_signal), \
                            128 | AZOUK_SIGNALS_c_name(SIG)));
		    BOOST_PP_SEQ_FOR_EACH(AZOUK_SIGNALS_connect_killers, ~,
                            AZOUK_SIGNALS_QUIT_ON_SEQ);
